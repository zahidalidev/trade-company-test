...
const BubbleView = ({ activeTab, data, links, logos }) => {
  ...
  useEffect(() => {
    ...
    svg
      ...
      .attr("cx", 0)
      .attr("cy", 0)
      .attr("r", 20); // Increased bubble size here
      ...
      .attr(
        ...
        (d) =>
          -Math.min(
            20,
            activeTab === "companies"
              ? getNameLength(d.name) * 2
              : getNameLength(d.firstname) * 2
          )
      )
      .attr("y", -20)
      .attr("width", (d) =>
        Math.min(
          40,
          activeTab === "companies"
            ? getNameLength(d.name) * 4
            : getNameLength(d.firstname) * 4
        )
      )
      .attr("height", 40)
    ...
    const moveBubbles = () => {
      nodes.attr("transform", function (d, i) {
        ...
        if (d.x < -30) d.x = width + 30; // Adjusted bubble size here
        else if (d.x > width + 30) d.x = -30; // Adjusted bubble size here
